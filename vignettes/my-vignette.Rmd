...
We used a GitHub repository to store the package, most recent updates and bug fixes will be there.
One will require the `devtools` package to do the installation.
...
How does the cofidence change based on the number of matings we execute? Maybe, there is a more optimal choice? 
```{r}
probabilitiesPlot(n_breedings_multi)
```
...
- The fertility of a mouse is 70%.
