...
#' distribution with probability of success being \code{fertility_p}
...
calculate_needed_breedings_textbook <- function(
    ...
    stopifnot(fertility_p <= 1 & fertility_p > 0)
    ...
    if(textbook_error){
        ...
        Nbreedings <- search_interval[
           min(which(pbinom(
               ...
               prob = fertility_p, lower.tail = F) >= confidence_p))
           ]
    }
    return(Nbreedings)
}
